...
function drawBarGraph (data, config) {
  ...
  const varianten = [
    ...
    'NBNL2023_nationale_drijfveren', 'NBNL2023_internationale_ambitie', 'PBLWLO2025_HOOGSNEL', 'PBLWLO2025_LAAGSNEL', 'PBLWLO2025_HOOGVERTRAAGD', 'PBLWLO2025_LAAGVERTRAAGD',
  ]
  ...
  config.scenarios.forEach((scenarioConfig, index) => {
    ...
    if (!yearMatch) {
      console.log(`Scenario title doesn't match expected pattern: ${scenarioConfig.title}`)
      return
    }
    ...
    // IMPORTANT: Check more specific patterns first (longest to shortest) to avoid substring matching issues
    ...
    if (scenarioType.includes('TNOAT2024_ADAPT')) displayName = 'TNOAT2024_ADAPT'
      ...
      console.log(`Unrecognized scenario type: ${scenarioType} (from ${scenarioConfig.title})`)
    }
 ...
 const categoryInfo = {
  ...
  function updateGraph () {
    ...
    varianten.forEach((scenarioName) => {
      ...
      // Skip if scenario data is not available
      if (!scenarioDataForYears) {
        console.log(`No data found for scenario: ${scenarioName}, skipping`)
        return
      }
